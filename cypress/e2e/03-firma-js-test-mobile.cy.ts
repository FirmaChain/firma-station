...
describe('Firma JS Mobile Test', () => {
	...
	proposalTests.forEach((test, index) => {
		it(`📱 ${test.testId}: Connect to Mobile ${test.label} Proposal test`, () => {
			...
			cy.get('[data-testid="proposal-description-textarea"]').clear().type(description).should('have.value', description);
			...
			if (test.type === 'COMMUNITY_POOL_SPEND_PROPOSAL') {
				...
				cy.get('[data-testid="proposal-recipient-input"]').clear().type(recipientAddress).should('have.value', recipientAddress);
				...
				cy.get('[data-testid="proposal-upgrade-name-input"]').clear().type(upgradeName).should('have.value', upgradeName);
			...
			cy.get('[data-testid^="proposal-id-"]')
				...
				.then(($firstProposal) => {
					...
					cy.get('body').then(($body) => {
						...
						if ($body.text().includes(expectedDescription) || $body.text().includes(`${prefix}_${test.label}_DESCRIPTION`)) {
