...
const ProposalDetailCard = ({ proposalState }: IProps) => {
	...
	const isSpendCommunityPools = (type: string) => getProposalTypeTypo(type) === PROPOSAL_MESSAGE_TYPE_COMMUNITYPOOLSPEND;
	...
	const getCommunityPoolAmounts = (message: IProposalMessageItem): IProposalAmount[] => {
		const amountListFromExtraData = Array.isArray((message.extraData as any)?.amounts) ? (message.extraData as any).amounts : [];
	...
	const getDisplayAmountText = (amount: IProposalAmount) => {
		...
		const amountSymbol = isMainDenom ? CHAIN_CONFIG.PARAMS.SYMBOL : (amount.denom || CHAIN_CONFIG.PARAMS.DENOM).toUpperCase();
	...
	const renderExplorerAddress = (address?: string) => {
		...
		return (
			<ExplorerLink href={`${CHAIN_CONFIG.EXPLORER_URI}/accounts/${normalizedAddress}`} target="_blank" rel="noopener noreferrer">
				{normalizedAddress}
			</ExplorerLink>
		);
	};
