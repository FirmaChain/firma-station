...
const Button = styled.button<{ disabled?: boolean; variant?: 'primary' | 'secondary' }>`
	...
	color: ${(props) => (props.variant === 'secondary' ? '#efefef' : 'white')};
...
const CreateWallet: React.FC<CreateWalletProps> = ({ onBack }) => {
	...
	const handleConfirmAndSave = async () => {
		...
		try {
			...
			const success = await recoverWalletFromMnemonic(mnemonic, walletName.trim(), password);
	...
	if (step === 'confirm') {
		return (
			<Container>
				...
				<FormContainer>
					<SuccessBox>
						<h3 style={{ margin: '0 0 12px 0', color: '#166534' }}>✓ Your wallet has been created!</h3>
					...
					<InfoBox>
						Your wallet has been successfully created and stored in offline mode. You can now use it to sign transactions or
						export your keys.
					</InfoBox>

					<Button onClick={onBack}>Return to Offline Mode Menu</Button>
				</FormContainer>
			</Container>
		);
	}
	...
	if (step === 'mnemonic') {
		return (
			<Container>
				...
				<FormContainer>
					<WarningBox>
						<strong>⚠️ Important:</strong> Write down these words in order and store them safely. This is the ONLY way to
						recover your wallet. Never share this phrase with anyone!
					</WarningBox>

					<MnemonicDisplay>
						<MnemonicGrid>{renderMnemonicWords()}</MnemonicGrid>
					...
					<CheckBox>
						...
						/>
						<label htmlFor="confirmSave">I have safely written down and stored my recovery phrase</label>
					</CheckBox>

					<Button onClick={handleConfirmAndSave} disabled={!confirmedSave || isLoading}>
	...
	return (
		<Container>
			...
			<FormContainer>
				<InfoBox>
					Create a new wallet with a randomly generated mnemonic phrase. Make sure to save your mnemonic phrase in a secure
					location.
				</InfoBox>

				<InputGroup>
					<Label>Wallet Name</Label>
					<Input value={walletName} onChange={(e) => setWalletName(e.target.value)} placeholder="Enter a name for your wallet" />
