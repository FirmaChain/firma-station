...
const Button = styled.button<{ variant?: 'danger' | 'secondary'; disabled?: boolean }>`
	...
	&:hover:not(:disabled) {
		background: ${(props) => (props.variant === 'secondary' ? '#f5f5f5' : props.variant === 'danger' ? '#DC2626' : '#2563EB')};
	}
`;
	...
	return (
		<>
			<Container>
				...
				<ContentContainer>
					<WarningBox>
						<strong>⚠️ Warning:</strong> Disconnecting your wallet will remove all wallet data from this browser. Make sure you
						have backed up your mnemonic phrase and/or private key before proceeding.
					</WarningBox>
					...
					<ChecklistContainer>
						...
						<ChecklistItem>
							<input type="checkbox" checked={checklist.backup} onChange={() => handleChecklistChange('backup')} />I have
							backed up my mnemonic phrase
						</ChecklistItem>

						<ChecklistItem>
							<input type="checkbox" checked={checklist.privateKey} onChange={() => handleChecklistChange('privateKey')} />I
							have saved my private key (if needed)
						</ChecklistItem>

						<ChecklistItem>
							<input type="checkbox" checked={checklist.understand} onChange={() => handleChecklistChange('understand')} />I
							understand that I will need my backup to access this wallet again
						</ChecklistItem>
					</ChecklistContainer>

					<InfoBox>
						💡 Tip: Use the "Export Mnemonic" or "Export Private Key" features before disconnecting if you haven't already
						backed up your wallet.
					</InfoBox>
