...
const NewProposalModal = () => {
	...
	const newProposalTx = (resolveTx: () => void, rejectTx: () => void, gas = 0) => {
		switch (proposalType) {
			...
			case 'STAKING_PARAMS_UPDATE_PROPOSAL':
				...
				submitStakingParamsUpdateProposal(title, description, Number(initialDeposit), stakingParams, stakingMetadata, gas)
					.then(() => {
						setUserData();
						resolveTx();
					})
	...
	const getMaxAmount = () => {
		const value = convertNumber(makeDecimalPoint(convertNumber(balance) - convertToFctNumber(getDefaultFee(isLedger, isMobileApp)), 6));
	...
	const nextStep = async () => {
		...
		const isCommunityPoolInvalid = proposalType === 'COMMUNITY_POOL_SPEND_PROPOSAL' && (recipient === '' || amount === 0);
		...
		if (isCommonInvalid || isCommunityPoolInvalid || isStakingParamsInvalid || isGovParamsInvalid || isSoftwareUpgradeInvalid) {
	...
	return (
		<Modal visible={newProposalState} closable={true} visibleClose={false} onClose={closeModal} width={newProposalModalWidth}>
						...
						<ModalInputWrap>
							...
							>
								...
								<p style={{ color: '#ccc', fontSize: '12px', margin: '0' }}>
									This proposal will cancel any pending software upgrade that has been scheduled but not yet executed. Use
									this when you need to halt a previously approved upgrade proposal.
								</p>
							</div>
						</ModalInputWrap>
					)}
