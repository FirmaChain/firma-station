...
export const ConfirmInput = styled.div<{ $point?: boolean }>`
	...
	${(props) => props.$point && `font-size:${props.theme.sizes.modal20};color:white;& > span { font-size: ${props.theme.sizes.modal16};`}
`;
...
export const GuideIcon = styled.div<{ step: number }>`
	width: 54px;
	height: 54px;
	background-image: url('${(props) => props.theme.urls[`mobileLoginStep${props.step}` as keyof typeof props.theme.urls]}');
	background-size: contain;
	background-position: center;
	margin-bottom: 10px;
`;
...
export const InputBoxDefault = React.forwardRef<HTMLInputElement, ComponentProps<'input'> & { decimal?: number; isInvalid?: boolean }>(
	(p, forwardedRef) => {
