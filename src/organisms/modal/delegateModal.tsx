...
const DelegateModal = () => {
	...
	const onChangeAmount = (e: any) => {
		...
		setAmount(amount);
		setActiveButton(convertNumber(amount) > 0 && convertNumber(amount) <= convertNumber(availableAmount) + convertNumber(rewardAmount));
	};
	...
	return (
		<Modal visible={delegateModalState} closable={true} onClose={closeModal} visibleClose={false} width={delegateModalWidth}>
			<ModalContainer>
				<ModalTitle>
					Delegate
					...
					<ModalToggleWrapper>
						...
						{isSafety && (
							<ModalTooltipWrapper>
								<ModalTooltipIcon />
								<ModalTooltipTypo>
									The entire amount is automatically entered except 0.1{CHAIN_CONFIG.PARAMS.SYMBOL}, which will be used as
									a transaction fee.
								</ModalTooltipTypo>
							</ModalTooltipWrapper>
						)}
