...
const UndelegateModal = () => {
	...
	useEffect(() => {
		setAvailableAmount(isValid(modalData.data) ? convertNumberFormat(convertToFctNumber(modalData.data.delegation.amount), 3) : '0');
	...
	const onClickMaxAmount = () => {
		...
		setAmount(amount);
		setActiveButton(convertNumber(amount) > 0 && convertNumber(amount) <= convertToFctNumber(modalData.data.delegation.amount));
	};
	...
	return (
		<Modal visible={undelegateModalState} closable={true} visibleClose={false} onClose={closeModal} width={undelegateModalWidth}>
			<ModalContainer>
				<ModalTitle>
					Undelegate
				...
				<ModalContent>
					...
					<ModalInputRowWrap>
						...
						<ModalValue>
							{isLedger
								...
								: `${convertToFctString(getDefaultFee(isLedger, isMobileApp).toString())} ${CHAIN_CONFIG.PARAMS.SYMBOL}`}
						</ModalValue>
					</ModalInputRowWrap>
					...
					<ModalTooltipWrapper>
						<ModalTooltipIcon />
						<ModalTooltipTypo>
							A 21 day period is required when undelegating your tokens. During the 21 day period, you will not receive any
							rewards. And you can't send and delegate that amount during 21 days.
						</ModalTooltipTypo>
					</ModalTooltipWrapper>
