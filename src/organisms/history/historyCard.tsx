...
import { HeaderColumn, HeaderWrapper, HistoryTypeBox, ItemColumn, ItemWrapper, ListWrapper } from './styles';
...
const Row = ({ data, index, style }: any) => {
	...
	if (index === historyList.length) {
		return (
			<ItemWrapper style={style}>
				<ItemColumn style={{ textAlign: 'center', gridColumn: '1 / -1' }}>{loading ? 'Loading more...' : ''}</ItemColumn>
			</ItemWrapper>
		);
	}
	...
	return (
		<ItemWrapper style={style} data-testid={`send-history-item-${index}`}>
			...
			<ItemColumn className="clamp-single-line" data-testid={`send-history-memo-${index}`}>
				{getMemo(currentHistory.memo)}
			</ItemColumn>
