...
const Row = ({ data, index, style, totalStakingState }: any) => {
	...
	const getReward = (validatorAddress: string) => {
		const reward = totalStakingState.stakingRewardList.filter((reward: any) => reward.validator_address === validatorAddress);
...
const DelegationCard = ({ totalStakingState, grantDataState }: IProps) => {
	...
	useEffect(() => {
		if (isInit && totalStakingState.delegationList.length > 0) {
			ky.get(`${CHAIN_CONFIG.RESTAKE.API}/restake/reward/${address}`)
	...
	const getParseRestakeDataList = (
		...
		for (const delegate of totalStakingState.delegationList) {
			...
			const latestRewards = latestReward.find((latestReward) => latestReward.validatorAddr === delegate.validatorAddress);
	...
	const onClickWithdrawAll = () => {
		if (isMobileApp && totalStakingState.delegationList.length > 3) {
			enqueueSnackbar('Withdraw can only be up to 3 validators using the mobile station. Please try mobile station app.', {
				variant: 'error',
	...
	const onClickRestake = () => {
		if (totalStakingState.delegationList.length > 0 && CHAIN_CONFIG.RESTAKE.API) {
			ky.get(`${CHAIN_CONFIG.RESTAKE.API}/restake/info`)
