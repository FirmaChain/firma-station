...
const sortedAmountDescending = (list: any[]) => [...list].sort((a, b) => (a.amount > b.amount ? -1 : a.amount < b.amount ? 1 : 0));
	...
	return (
		<Link to={{ pathname: `${CHAIN_CONFIG.EXPLORER_URI}/accounts/${item.delegatorAddress}` }} target={'_blank'}>
			<ItemWrapper style={style}>
				...
				<ItemColumn>{`${convertNumberFormat(convertToFctNumber(item.amount), 3)} ${CHAIN_CONFIG.PARAMS.SYMBOL}`}</ItemColumn>
			</ItemWrapper>
		</Link>
	);
};
...
const RedelegatorRow = ({ data, index, style }: any) => {
	...
	return (
		<ItemWrapper style={style}>
			...
			<ItemColumn
				style={NumericCell}
			>{`${convertNumberFormat(convertToFctNumber(item.balance), 3)} ${CHAIN_CONFIG.PARAMS.SYMBOL}`}</ItemColumn>
