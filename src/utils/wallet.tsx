...
import { clearKey, getRandomKey, isInvalidWallet, restoreWallet, storeWallet } from './keyBridge';
...
function useFirma() {
	...
	const sendToken = async (address: string, amount: string, tokenID: string, decimal: number, memo = '', estimatedGas: number) => {
		const result = await FirmaSDK.sendToken(address, tokenID, convertNumber(amount), decimal, memo, estimatedGas);

		checkValidateResult(result);
	};

	const getGasEstimationSendToken = async (address: string, amount: string, tokenID: string, decimal: number, memo = '') => {
		return await withGasEstimationLoader(() =>
			FirmaSDK.getGasEstimationSendToken(address, tokenID, convertNumber(amount), decimal, memo)
		);
	};

	const sendIBC = async (address: string, amount: string, denom: string, decimal: number, memo = '', estimatedGas: number) => {
		const result = await FirmaSDK.sendIBC(address, convertNumber(amount), denom, decimal, memo, estimatedGas);

		checkValidateResult(result);
	};

	const getGasEstimationsendIBC = async (address: string, amount: string, denom: string, decimal: number, memo = '') => {
		return await withGasEstimationLoader(() => FirmaSDK.getGasEstimationSendIBC(address, convertNumber(amount), denom, decimal, memo));
	};
	...
	const getGasEstimationRedelegate = async (validatorAddressSrc: string, validatorAddressDst: string, amount: number) => {
		return await withGasEstimationLoader(() => FirmaSDK.getGasEstimationRedelegate(validatorAddressSrc, validatorAddressDst, amount));
	};
	...
	const submitParameterChangeProposal = async (
		...
		const result = await FirmaSDK.submitParameterChangeProposal(title, description, initialDeposit, paramList, estimatedGas);
	...
	const submitCommunityPoolSpendProposal = async (
		...
		const result = await FirmaSDK.submitCommunityPoolSpendProposal(title, description, initialDeposit, amount, recipient, estimatedGas);
	...
	const getGasEstimationSubmitTextProposal = async (title: string, summary: string, initialDeposit: number) => {
		return await withGasEstimationLoader(() => FirmaSDK.getGasEstimationSubmitTextProposal(title, summary, initialDeposit));
	};
	...
	const getGasEstimationSubmitSoftwareUpgrade = async (
		...
		return await withGasEstimationLoader(() =>
			FirmaSDK.getGasEstimationSubmitSoftwareUpgradeProposalByHeight(title, description, initialDeposit, upgradeName, height)
		);
	};

	const submitCancelSoftwareUpgrade = async (title: string, description: string, initialDeposit: number, estimatedGas: number) => {
		const result = await FirmaSDK.submitCancelSoftwareUpgradeProposal(title, description, initialDeposit, estimatedGas);

		checkValidateResult(result);
	};

	const getGasEstimationSubmitCancelSoftwareUpgrade = async (title: string, description: string, initialDeposit: number) => {
		return await withGasEstimationLoader(() =>
	...
	const submitStakingParamsUpdateProposal = async (
		...
		const result = await FirmaSDK.submitStakingParamsUpdateProposal(title, summary, initialDeposit, params, metadata, estimatedGas);
	...
	const submitGovParamsUpdateProposal = async (
		...
		const result = await FirmaSDK.submitGovParamsUpdateProposal(title, summary, initialDeposit, params, metadata, estimatedGas);
	...
	const grantStakeAuthorizationDelegate = async (
		...
		const result = await FirmaSDK.grantStakeAuthorizationDelegate(validatorAddressList, expirationDate, maxFCT, estimatedGas);
